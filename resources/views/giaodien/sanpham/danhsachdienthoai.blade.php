  ...
  <div class="section-padding">
    <div class="container">
      <div class="row">
        <div class="product-filter">
          <div class="col-md-6">
            ...
            <span class="filter-text">Hiển thị {{$dienthoai->firstItem()}}-{{$dienthoai->lastItem()}} của {{$dienthoai->total()}}</span>
          </div>
      ...
      <div class="pagination pagination-02 text-center">
        @if($dienthoai->currentPage() != 1)
        <a href="{!! str_replace('/?','?',$dienthoai->appends(['sort' => $sort])->url($dienthoai->currentPage() -1))!!}" class="prev"><i class="ti-angle-double-left"></i></a>        
        @endif 
        @for($i = 1; $i <= $dienthoai->lastPage(); $i=$i+1)
        <a href="{!! str_replace('/?','?',$dienthoai->appends(['sort' => $sort])->url($i))!!}" class="{!! ($dienthoai->currentPage() == $i)? 'active' :''!!}">{!!$i!!}</a>
        @endfor 
         @if($dienthoai->currentPage() != $dienthoai->lastPage())
          <a href="{!! str_replace('/?','?',$dienthoai->appends(['sort' => $sort])->url($dienthoai->currentPage() + 1))!!}" class="next"><i class="ti-angle-double-right"></i></a> 
         @endif
      </div>
      <!-- /.pagination -->
