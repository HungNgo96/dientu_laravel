...
@section('content')
@include('layout.page',['data'=>$loaihsx->tenhangsanxuat])
<!-- /.page-name-sec -->
  ...
  <div class="section-padding">
    <div class="container">
      <div class="row">
        <div class="product-filter">
          <div class="col-md-6">
            ...
            <span class="filter-text">Hiển thị {{$sanphamhsx->firstItem()}}-{{$sanphamhsx->lastItem()}} của {{$sanphamhsx->total()}}</span>
          </div>
          {{--  //sapxep  --}}
      ...
      <div class="shop-products">
        <div class="tab-content">
          ...
          <div role="tabpanel" class="tab-pane fade text-center active in" id="grid">
              ...
              @endphp
            @if(count($sanphamhsx)>0)
                @foreach($sanphamhsx as $item)
                  @php
      ...
      <div class="pagination pagination-02 text-center">
         ...
         @if($sanphamhsx->currentPage() != 1)
        <a href="{!! str_replace('/?','?',$sanphamhsx->appends(['sort' => $sort])->url($sanphamhsx->currentPage() -1))!!}" class="prev"><i class="ti-angle-double-left"></i></a>        
        @endif 
        @for($i = 1; $i <= $sanphamhsx->lastPage(); $i=$i+1)
        <a href="{!! str_replace('/?','?',$sanphamhsx->appends(['sort' => $sort])->url($i))!!}" class="{!! ($sanphamhsx->currentPage() == $i)? 'active' :''!!}">{!!$i!!}</a>
        @endfor 
         @if($sanphamhsx->currentPage() != $sanphamhsx->lastPage())
          <a href="{!! str_replace('/?','?',$sanphamhsx->appends(['sort' => $sort])->url($sanphamhsx->currentPage() + 1))!!}" class="next"><i class="ti-angle-double-right"></i></a> 
         @endif
